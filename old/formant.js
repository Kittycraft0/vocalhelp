...
function drawFormants() {
    ...
    //formantCtx.clearRect(formantCanvas.width - 1, 0, 2, formantCanvas.height); // Draw a vertical line on the right edge
    ...
    */
    ...
    for (let i = 0; i < finalPoint.length; i++) {
        ...
        if (data.visualType === "linear") {
            ...
            //formantCtx.fillStyle = pointColors[pointColors.length-1-i]; //frequencyToColor(finalPoint[i]);
        }
        formantCtx.fillRect(formantCanvas.width - 1, y, 1, 5); // Draw rectangles with width and height of 1 pixel
...
function visualizeFormants(formantFrequencies) {
    ...
    // Scroll the image left
    const imageData = spectrogramCtx.getImageData(1, 0, formantCanvas.width - 1, formantCanvas.height);
    formantCtx.putImageData(imageData, 0, 0);
    ...
    formantFrequencies.forEach((frequency, index) => {
        ...
        formantCtx.fillStyle = color;
        //formantCtx.fillRect(formantCanvas.width - 1 - index * 10, y, 10, 10);
        formantCtx.fillRect(formantCanvas.width, y, 10, 10);
    });
}
